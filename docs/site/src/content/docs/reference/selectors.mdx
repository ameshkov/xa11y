...
report that state (Linux and Windows cannot report `maximized` or
`fullscreen` at all). An element whose state is unknown matches nothing, so
`[minimized='false']` and `[minimized='true']` both exclude windows the
platform cannot describe:
