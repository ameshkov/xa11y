...
timeout error rather than reporting success. A window whose `AXFullScreen`
is not settable has no fullscreen state to settle, so `Restore` only clears
`AXMinimized` for it and returns without polling.
