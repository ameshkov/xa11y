...
| `Maximize` | set AXFullScreen=true *(native fullscreen)* | *(unsupported)* | WindowPattern.SetWindowVisualState(Maximized) |
...
### `Maximize`: native fullscreen is the state

macOS exposes native fullscreen through the `AXFullScreen` attribute, the
only window state behind `Maximize` that can be both read and written.
`AXMinimized` is readable and writable too, but it is the state `Minimize`
drives; `AXFullScreen` is what lets `Maximize` and `Restore` be idempotent.
`Maximize` sets it to `true` and `Restore` sets it to `false`. An
already-fullscreen window keeps `maximize` whether or not `AXFullScreen` is
settable, so a repeated `maximize` stays a no-op. A window that is not
fullscreen and cannot be set fullscreen reports `maximize` as
`ActionNotSupported`, never a silent no-op.

The green zoom button is deliberately not used. Its `AXPress` and
`AXZoomWindow` actions toggle the window, so a second `maximize` would exit
fullscreen, and there is no readable zoom state to detect that. A window
whose green button only classic-zooms (System Settings is one) does not
advertise `maximize` at all, because classic zoom is not the state the verb
promises.
...
than queued. On a fullscreen-capable window both verbs poll the
application's window set and re-issue the absolute set until the promised
state holds consistently; a transition that never commits fails with a
timeout error rather than reporting success. A window that is not fullscreen
and whose `AXFullScreen` is not settable has no fullscreen state to settle,
so `Restore` only clears `AXMinimized` for it and returns without polling.
A window that reads fullscreen but cannot clear the state does not advertise
`restore` at all.
`maximized` cannot be read on macOS, so the state is reported as
`fullscreen`. `AXZoomed` matches the AppKit *property* `NSWindow.isZoomed`,
is not declared in the SDK headers, and live windows answer
