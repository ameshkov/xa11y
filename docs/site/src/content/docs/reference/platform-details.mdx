...
`Maximize` sets it to `true` and `Restore` sets it to `false`. A window
that cannot enter fullscreen (its `AXFullScreen` is not settable) reports
`maximize` as `ActionNotSupported`, never a silent no-op.
...
than queued. Both verbs poll the application's window set and re-issue the
absolute set until the promised state holds consistently; a transition that
never commits fails with a timeout error rather than reporting success.
