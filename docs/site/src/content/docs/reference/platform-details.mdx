...
only window state whose value can be both read and written. `Maximize` sets
it to `true` and `Restore` sets it to `false`. A window that cannot enter
fullscreen (its `AXFullScreen` is not settable) reports `maximize` as
