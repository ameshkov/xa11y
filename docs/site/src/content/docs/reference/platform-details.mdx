...
| xa11y Action | macOS | Linux (AT-SPI) | Windows (UIA) |
| --- | --- | --- | --- |
| `Raise` | AXRaise | Component.GrabFocus *(same path as Focus)* | restore if minimized, then SetForegroundWindow and SetFocus |
| `Minimize` | set AXMinimized=true | *(unsupported)* | WindowPattern.SetWindowVisualState(Minimized) |
| `Maximize` | set AXZoomed=true | *(unsupported)* | WindowPattern.SetWindowVisualState(Maximized) |
| `Restore` | clear AXMinimized / AXZoomed | *(unsupported)* | WindowPattern.SetWindowVisualState(Normal) |
...
macOS reads all three
(`AXMinimized` / `AXZoomed` / `AXFullScreen`) on window-like elements;
