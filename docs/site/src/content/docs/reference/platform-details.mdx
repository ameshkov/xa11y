...
always `None` on macOS. After a `maximize`, `fullscreen` reports the state
instead.
...
fullscreen state to settle, so `Restore` only clears `AXMinimized` and
returns without polling. A window that reads fullscreen but cannot clear the
state does not advertise `restore` at all.
