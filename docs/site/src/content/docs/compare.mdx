...
[Strands](https://strandsagents.com) is AWS's open-source SDK for building agents in Python or TypeScript. A capability there is a tool, which is a function whose schema the model sees and calls when it decides to. [strands-xa11y](/guides/strands/) (`pip install strands-xa11y`) is the desktop in that shape. It packages the same loop as a single `use_desktop` tool, so an agent gains desktop control by listing it in `tools=[...]` instead of shelling out to a CLI. The model reads a snapshot of the tree with a ref on every line, then acts by ref or by [selector](/reference/selectors/). Its `inspect_desktop` variant is the same tool with the acting half removed from the schema, for an agent that should observe and never touch. [Drive a desktop from a Strands agent](/guides/strands/) covers the loop and the consent prompts around acting on a real desktop. The [platform permissions](/guides/install/) underneath are xa11y's.
...
- **[strands-xa11y](/guides/strands/)** packages xa11y as a desktop-control tool for [Strands](https://strandsagents.com), AWS's open-source agent SDK. A read-only variant covers agents that observe and never act.
